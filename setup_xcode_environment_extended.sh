...
current_konan=$(cat $file_path|grep 'konan'|sed 's/<string>*//'|sed 's/ *<\/string>/ /'|sed 's/\t//'|sed 's/\t//'|sed 's/ *$//')
...
case $yn in
[Yy]*)(replace_paths $file_path $JAVA_HOME $ANDROID_SDK_ROOT $GRADLE_USER_HOME $KONAN_DATA_DIR)
exit
;;
[Nn]*)exit;;
